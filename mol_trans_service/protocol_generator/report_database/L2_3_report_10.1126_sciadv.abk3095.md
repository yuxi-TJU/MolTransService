...
## System 1: [n]CPP (n=5-12)
  ...
  - Interface:  
      - interface_geometry_text: Au pyramid apex binds directly to a C-C bond of a phenylene ring in an $eta^2$ ($pi$-coupled) fashion.
  - Variation_notes: This represents a series of molecules where *n* (the number of phenylene units) varies, e.g., from 5 to 12. The goal is to compare the transport properties across this series.
...
4.  Extract the transmission value at the Fermi level (E_F, which is the cluster HOMO energy printed to the screen) from each `Transmission.txt` file.
