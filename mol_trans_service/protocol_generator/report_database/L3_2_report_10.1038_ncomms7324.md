...
## System 1: $SC_6FcC_7S$ (Symmetric)
  ...
  - Interface:
      - interface_geometry_text: Thiolate-S binds to an Au(111) surface, adapted as a two-terminal junction. The paper's DFT uses Au(111).
  - Variation_notes: "Symmetric case. Ferrocene (Fc) unit is centered in the alkyl chain (n=6)."
...
## System 2: $SC_3FcC_{10}S$ (Asymmetric, R<1)
  ...
  - Variation_notes: "Asymmetric case. Fc unit is close to one electrode (n=3), corresponding to the experimental R<1 case."
...
5.  **EM Lengths**: The length of the extended molecule (L_EM) along the transport (z) direction must be measured from each of the two `.xyz` files (e.g., `L_sc3`, `L_sc6`) in Ångstroms. This value is a required input for the `current_parallel.py` script.
