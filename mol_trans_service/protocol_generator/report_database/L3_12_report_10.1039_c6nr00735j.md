...
## System 1: DPE-2F (Asymmetric)
  ...
  - Core Molecule:
      ...
      - core_smiles: Fc1cc(C#Cc2ccc(S)cc2)cc(F)c1CCc1ccc(C#Cc2ccc(S)cc2)cc1
  - Anchors:
  ...
  - Interface:
      - interface_geometry_text: Thiol-SH anchors to Au electrodes, formed via MCBJ. This is modeled as a Thiolate-S binding to an Au(111) trimer (hollow) site in a two-terminal junction.
  - Variation_notes: "Asymmetric molecule with fluorine substituents, expected to show rectification."
...
## System 2: DPE (Symmetric)
  ...
  - Core Molecule:
      - abbreviation: DPE
      - full_chemical_name: N/A
      - core_smiles: Sc1ccc(C#Cc2ccc(CCc3ccc(C#Cc4ccc(S)cc4)cc3)cc2)cc1
  - (Anchors, Electrodes, and Interface are the same as System 1)
  - Variation_notes: "Symmetric control molecule (no fluorine). Expected to be non-rectifying."
...
5.  **EM Lengths**: The length of the extended molecule (L_EM) along the transport (z) direction must be measured from each of the two `.xyz` files (e.g., `L_dpe2f`, `L_dpe`) in Ångstroms. This is a required input for the `current_parallel.py` script.
